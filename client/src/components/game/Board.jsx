  ...
  return (
    <div className="h-full bg-black text-white selection:bg-white/20">
      <div className="w-full h-full bg-black flex flex-col items-center justify-center font-sans p-4 relative">
        ...
        {/* Header */}
        <div className="w-full bg-neutral-900 border border-neutral-800 rounded-t-2xl shadow-2xl p-4 flex flex-col z-10 gap-4">
          <div className="flex flex-col md:flex-row items-start md:items-center justify-start gap-6">
                  ...
                  <div className="flex items-center justify-between mt-auto gap-4">
                    ...
                    <div className="flex gap-1" title={`${p.lives} lives`}>
                      {Array.from({ length: Math.max(3, p.lives) }).map((_, i) => (
                        <span key={i}>{p.shieldCount > i ? '🩵' : i < p.lives ? '❤️' : '🖤'}</span>
                      ))}
                    </div>
                  </div>
                </div>
            ...
            <div className='flex-1'>
              {snapshot.phase === 'paused' && <div className="text-2xl font-semibold uppercase text-center">Paused by {snapshot.pausedBy.name}</div>}
            </div>

            {/* Game info */}
            <div className="flex gap-6 items-stretch justify-end">
              ...
              <div className="flex flex-col items-end whitespace-nowrap bg-black/50 px-5 py-3 rounded-lg border border-neutral-800">
                ...
                <span className="font-mono text-3xl text-neutral-100 font-bold">{snapshot.gameMode === 'tailHunt' ? 'Tail Hunt' : 'Classic'}</span>
              </div>
        ...
        {/* Board */}
        <div className="flex-1 w-full bg-neutral-950 border border-t-0 border-neutral-800 rounded-b-2xl shadow-2xl z-10 flex min-h-0 overflow-hidden">
